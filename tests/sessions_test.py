...
def test_user(client: TestClient) -> list[User]:
    user_data = {
    ...
    user = UserCreatedResponse.model_validate(client.post("/users/", json=user_data).json())
    return user

def test_complete_session_workflow(db_session, test_user):
...
def test_device_detection(db_session, test_user):
    service = SessionService(db_session)
...
def test_multiple_sessions_and_cleanup( db_session, test_user):
    service = SessionService(db_session)
    
    tokens = []
    for i in range(3):
        token = f"multi_session_token_{i}"
        _ = service.create_user_session(
            user_id=test_user.id,
            token=token,
...
def test_repository_direct_access( db_session, test_user):
