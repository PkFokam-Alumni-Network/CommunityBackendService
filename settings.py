...
class Settings:
    def __init__(self):
        ...
        if self.ENV == "production":
            secret_name = "paci/backend/creds"
            ...
            self.SECRET_KEY = secrets.get("SECRET_KEY", "DEFAULT_KEY")  # fallback if not stored
            self.ACCESS_KEY = secrets.get("ACCESS_KEY", "DEFAULT_KEY")  # fallback if not stored
            self.BUCKET_NAME = secrets.get("BUCKET_NAME")
