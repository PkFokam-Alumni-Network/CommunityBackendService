...
class UserEventRepository(metaclass=SingletonMeta):
    ...
    @retry_on_db_error()
    def remove_user_from_event(self, db: Session, user_email: String, event_id: int) -> None:
        try:
            user = db.query(User).filter(User.email == user_email).first()
            if not user:
                raise ValueError(f"User with email {user_email} does not exist.")
            user_event = db.query(UserEvent).filter_by(user_id=user.id, event_id=event_id).first()
            if user_event:
    ...
    def get_users_for_event(self, db: Session, event_id: int) -> list[User]:
        return db.query(User).join(UserEvent).filter(UserEvent.event_id == event_id).all()
    ...
    def get_events_for_user(self, db: Session, user_email: String) -> list[Event]:
        ...
        if not user:
                raise ValueError(f"User with email {user_email} does not exist.")
        return db.query(Event).join(UserEvent).filter(UserEvent.user_id == user.id).all()

    @retry_on_db_error()
    def is_user_registered_for_event(self, db: Session, user_email: String, event_id: int) -> bool:
        user = db.query(User).filter(User.email == user_email).first()
        if not user:
            raise ValueError(f"User with email {user_email} does not exist.")
